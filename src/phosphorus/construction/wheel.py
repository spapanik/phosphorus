...
class WheelBuilder(Builder):
    __slots__ = [*Builder.__slots__, "editable"]
    ...
    def build(self) -> Path:
        ...
        wheel = self.output_dir.joinpath(self.wheel_filenames[tag])
        ...
        with TemporaryDirectory() as temp_dir_name:
            ...
            self.write_files(files, wheel, temp_dir)

        return wheel

    @property
    def wheel_base_name(self) -> str:
        package_name = self.meta.package.distribution_name
        return f"{package_name}-{self.meta.version}"

    @property
    def dist_info(self) -> str:
        return f"{self.wheel_base_name}.dist-info"

    @property
    def wheel_filenames(self) -> dict[Tag, str]:
        return {tag: f"{self.wheel_base_name}-{tag}.whl" for tag in self.meta.tags}
    ...
    def write_files(
        self, files: dict[Path, ZippedFile], wheel: Path, temp_dir: Path
    ) -> None:
        with ZipFile(wheel, mode="w", compression=ZIP_DEFLATED) as zip_file:
            rows = []
            ...
            record_info = ZippedFile.from_file(
                source=record, target=Path(self.dist_info).joinpath("RECORD")
            )
            zip_file.writestr(
                record_info.zip_info, record.read_bytes(), compress_type=ZIP_DEFLATED
            )
