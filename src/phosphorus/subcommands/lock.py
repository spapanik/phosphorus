...
class LockCommand(BaseCommand):
    ...
    @staticmethod
    def write_dependency_to_lockfile(dependency: LockEntry, lockfile: TextIO) -> None:
        lockfile.write("\n[[packages]]\n")
        lockfile.write(f'name = "{dependency.package.distribution_name}"\n')
        lockfile.write(f'version = "{dependency.version}"\n')
        lockfile.write(f"groups = {dependency.groups}\n")
        if marker := str(dependency.marker):
            lockfile.write(f'marker = "{marker}"\n')
        lockfile.write("hashes = [\n")
        for file_hash in dependency.hashes:
