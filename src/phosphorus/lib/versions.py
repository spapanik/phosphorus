...
class Epoch:
    ...
    @classmethod
    def from_string(cls, match: Match) -> Self:
...
class Release:
    ...
    @classmethod
    def from_string(cls, match: str) -> Self:
    ...
    def padded(self, zeroes: int) -> Self:
        n = len(self.release)
        if zeroes < n:
...
class Pre:
    ...
    @classmethod
    def from_string(cls, match_letter: Match, match_number: Match) -> Self:
...
class VersionClause:
    ...
    def is_superset(self, clauses: Iterable[Self]) -> bool:
        # TODO: Implement this
        return all(clauses)
    ...
    def match(self, candidate: Version) -> bool:
        ...
        match self.operator:
            ...
            case ComparisonOperator.EXACT_MATCH:
                return self.match_exact(candidate)
            case _:
                msg = f"`{self.operator}` should not be possible"
                raise UnreachableCodeError(msg)
