...
from phosphorus.lib.types import Match, VersionOperator
...
class Epoch:
    ...
    @classmethod
    def from_string(cls, match: Match) -> Epoch:
...
class Release:
    ...
    def __init__(self, full_release: tuple[int, ...]):
        release = tuple(
    ...
    @classmethod
    def from_string(cls, match: str) -> Release:
    ...
    def padded(self, zeroes: int) -> Release:
        n = len(self.release)
        if zeroes < n:
...
class Pre:
    ...
    @classmethod
    def from_string(cls, match_letter: Match, match_number: Match) -> Pre:
...
class Post:
    ...
    @classmethod
    def from_string(cls, match_letter: Match, match_number: Match) -> Post:
...
class Dev:
    ...
    @classmethod
    def from_string(cls, match_letter: Match, match_number: Match) -> Dev:
...
class Local:
    ...
    @classmethod
    def from_string(cls, match: Match) -> Local:
        local_version = (
            tuple(
...
class Version:
    ...
    def base_version(self) -> Version:
        if self.is_base_version:
            return self
        return Version.from_string(f"{self.epoch}{self.release}")
    ...
    def padded(self, zeroes: int) -> Version:
...
class VersionClause:
    ...
    def __post_init__(self) -> None:
        ...
        if self.operator in {"~=", "<=", ">=", "<", ">"}:
            if self.identifier.local:
                ...
                raise RuntimeError(msg)
            if self.operator == "~=" and len(self.identifier.release.full_release) == 1:
                msg = (
                    "Compatible release operator is not permitted with single segments"
                )
                raise RuntimeError(msg)
        if self.operator not in {"==", "!="} and self.identifier.prefix_match:
    ...
    @classmethod
    def from_string(cls, clause: str) -> VersionClause:
        if clause.startswith("==="):
            operator_length = 3
            operator: VersionOperator = "==="
        elif clause[:2] in {"<=", "!=", "==", ">=", "~="}:
            operator_length = 2
            operator = clause[:2]  # type: ignore[assignment]
        elif clause[:1] in {"<", ">"}:
            operator_length = 1
            operator = clause[:1]  # type: ignore[assignment]
        else:
            operator_length = 0
            operator = "=="
    ...
    def match_compatible(self, candidate: Version) -> bool:
        ...
        return VersionClause(operator="==", identifier=wildcard).match(candidate)
